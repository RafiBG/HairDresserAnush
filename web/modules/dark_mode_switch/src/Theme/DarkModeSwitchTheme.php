...
namespace Drupal\dark_mode_switch\Theme;
...
/**
 * Dark mode switch theme negotiator.
 */
class DarkModeSwitchTheme implements ThemeNegotiatorInterface {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    $selected_theme = $this->configFactory->getEditable('dark_mode_switch.settings')->get('active_theme');
    if (!empty($selected_theme)) {
      ...
      if ($start_time - $current_time < 0 && $current_time - $end_time < 0) {
        $this->theme = $this->configFactory->getEditable('dark_mode_switch.settings')->get('active_theme');
      }
    }
  ...
  private function getThemeList() {
    $config = $this->configFactory->getEditable('dark_mode_switch.settings')->get('dark_mode');
    $themes_list = [];
